...
```{r}
scaled_data<-read_csv(here::here("data","cleaned_data","scaled_data.csv"))%>%
  dplyr::relocate(Departement, .after = Code)

kable(head(scaled_data,5))
...
sf_IDB%>%select(Code,Departement,IDB,IDBB)%>%mapView(zcol="IDBB")
...
departement_sf%>%drop_na()->departement_sf
full_join(departement_sf,final_data,by="Code")->sf_IDB
sf_IDB%>%select(Code,Departement,IDB,IDBB)%>%mapView(zcol="IDB")
